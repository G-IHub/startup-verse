...
function StatusDot({ variant, label }) {
  return (
    <span className="flex shrink-0 items-center gap-1.5">
      ...
      <span className="font-body text-xs text-v2-muted">{label}</span>
    </span>
  );
}
  ...
  return (
    <li className={participantCardClass()}>
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate font-body text-sm font-medium text-v2-heading">
          {name}
          {isLocal && (
            <span className="ml-1 font-normal text-v2-muted">(You)</span>
          )}
        </p>
        <StatusDot variant="in-call" label="In call" />
      </div>
      <div className="flex shrink-0 items-center gap-1 text-v2-muted">
        {micOn ? (
          <Mic className="h-3.5 w-3.5" aria-label="Microphone on" />
        ) : (
          <MicOff className="h-3.5 w-3.5 text-red-500" aria-label="Muted" />
        )}
        {camOn ? (
  ...
  return (
    <li className={participantCardClass()}>
      ...
      <Button
        type="button"
        size="sm"
        className="h-8 shrink-0 gap-1 bg-v2-blue px-2.5 text-xs text-white hover:bg-v2-blue-dark"
        disabled={cooldown}
...
export default function V2CallParticipantRoster() {
