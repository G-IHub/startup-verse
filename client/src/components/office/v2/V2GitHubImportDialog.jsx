...
/** V2 restyle of office/GitHubImportDialog.jsx — same real GitHub import flow, V2 tokens. */
export default function V2GitHubImportDialog({ open, onOpenChange, onImported }) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {loading && !issues.length && !repos.length ? (
          <p className="font-body text-sm text-v2-muted">Loading…</p>
        ) : !repo ? (
          <ul className="max-h-72 space-y-1 overflow-y-auto">
            {repos.map((row) => (
              <li key={row.id}>
                <button
                  type="button"
                  className="w-full rounded-lg px-3 py-2 text-left font-body text-sm hover:bg-v2-blue-tint"
              ...
              {issues.map((issue) => (
                <li key={issue.number} className="flex items-start gap-2">
                  ...
                  <label
                    ...
                    className="font-body text-sm text-v2-heading"
                  >
