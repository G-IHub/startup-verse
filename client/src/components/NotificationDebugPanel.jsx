...
export default function NotificationDebugPanel() {
  ...
  const testCreateNotification = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/notifications/test`,
        {
          ...
          body: JSON.stringify({
            ...
            message:
              "This is a test notification created manually at " +
      ...
      if (response.ok) {
        ...
        setResult(`✅ Success! Notification ID: ${data.notification?.id}`);
        toast.success("Test notification created!");
      } else {
        console.error("❌ [DEBUG] Failed to create notification:", data);
  ...
  const testFetchNotifications = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResult(`✅ Found ${data.notifications?.length || 0} notifications`);
        console.log("📋 [DEBUG] Notifications:", data.notifications);
      } else {
        console.error("❌ [DEBUG] Failed to fetch:", data);
        setResult(`❌ Error: ${JSON.stringify(data)}`);
      }
    } catch (error) {
      console.error("❌ [DEBUG] Error:", error);
  ...
  if (!user || user.role !== "founder") {
    return null; // Only show for founders in dev
  }
  return (
