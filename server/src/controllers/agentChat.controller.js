...
\`\`\`SPRINT_PLAN
{"milestones":[{"title":"...","description":"...","tasks":[{"title":"...","description":"...","buildTask":false,"filePath":"","assignAgentKey":null,"agentActionKey":null,"agentPayload":null}]}]}
\`\`\`
  ...
  **A task can instead be real Sales/Marketing work, decided the same way, at the same time.** If a task is something AI Sales or AI Marketing should actually do (not just a human to-do), set \`"assignAgentKey"\` to \`"sales"\` or \`"mkt"\`, \`"agentActionKey"\` to a real action for that agent (see the AGENT_TASK section below for the real ones), and \`"agentPayload"\` to that action's real payload. Once the founder approves the plan, this task hands itself off automatically — same "no further chat needed" principle as \`buildTask\`, and it closes itself out the same way once the real work finishes (or lands in the Approval Queue first, for the two real sends). A task is either \`buildTask\`, agent-assigned, or neither — never combine \`buildTask: true\` with a real \`assignAgentKey\` on the same task. Leave \`assignAgentKey\`/\`agentActionKey\`/\`agentPayload\` null for anything that's genuinely human work (talking to users, running interviews, making a judgment call) — don't assign real agent work just to make a task look automated.
...
\`\`\`ADD_TASKS
{"milestoneId":"...","tasks":[{"title":"...","description":"...","buildTask":false,"filePath":"","assignAgentKey":null,"agentActionKey":null,"agentPayload":null}]}
\`\`\`
  Same task shape as a sprint plan's tasks — set "buildTask":true and a real "filePath" for anything that should go straight to AI Developer once approved, or "assignAgentKey"/"agentActionKey"/"agentPayload" for real Sales/Marketing work (see the SPRINT_PLAN section above — identical rules, same real hand-off). This always needs the founder's approval before any task exists for real, same as everything else here.
