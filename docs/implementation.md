...
**2026-05-02 (A–Z remediation):** Invitation links route by token kind (cohort vs talent). Deliverables carry `type`; submissions use enum statuses plus `feedback` and `attachments`. Founder dashboard shows org announcements/events; org mentor admin includes `MentorManager`. Google: `GOOGLE_INTEGRATION_ENABLED=true` to allow future OAuth; otherwise explicit off state. Admin analytics: `GET /admin/analytics/snapshot`. Calendar: weekly outcomes from `GET /founders/:id/weekly-outcomes`. `Meeting.startupId` references `Startup`. File helper: `server/src/services/storage.js`. Email: `server/src/services/emailService.js` (log-only unless `EMAIL_TRANSPORT=smtp`).
...
**Phase 2.7 (check-ins) — implemented:** Daily check-ins use the existing **`POST /api/v1/startups/:startupId/activities`** with `type: "check-in"` and a short `message` (Virtual Office wraps the user’s text as `checked in: "…"`). The server assigns a **type-based icon** in stored metadata (e.g. check-in → ✅) and emits the usual realtime activity event. **`GET /startups/:startupId/activities`** supports optional **`type`** (e.g. `check-in`) plus `limit` for filtered lists. The Virtual Office dialog shows **today’s team check-ins** from the activity feed (local calendar day), refetches check-ins when opened, and only treats submit as successful after the POST succeeds (`hasCheckedInToday` is derived from persisted feed data for the current user).

**Phase 2.8 (interactive tour) — implemented:** Virtual Office uses **`react-joyride`** via **`InteractiveTour`** with steps in **`client/src/components/tours/tourSteps.jsx`**. Completion is stored on the user as **`virtualOfficeTourCompleted`** (writable via **`PUT /api/v1/auth/profile/:userId`** with boolean `true`; only admins may set `false`). The client also keeps **`localStorage[`tour_completed_virtual-office-tour`]`** after a successful finish. **First visit** (server flag false): **Skip** is hidden and does not persist completion; **Finish** runs the profile update then sets localStorage. **Replay** clears localStorage and uses a bypass flag so the tour can run again with Skip allowed. Legacy users with localStorage but no server flag get a one-time migration PUT on office load.
