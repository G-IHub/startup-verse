...
| team-member-dashboard | core-role | `client/src/components/dashboards/TeamMemberDashboard.jsx` | team-member | `GET /team-members/:teamMemberId/tasks`<br>`PUT /team-members/:teamMemberId/tasks/:taskId`<br>`GET/POST /team-members/:teamMemberId/status`<br>`GET /team-members/:teamMemberId/performance` | self/admin | low (optional subscriptions in surrounding office views) | REST reload + existing local cached task fallback |
| talent-dashboard | core-role | `client/src/components/dashboards/TalentDashboard.jsx` | talent | `GET /talent/startup-posts`<br>`GET/POST /talent/:talentId/applications`<br>`GET/POST /talent/:talentId/saved`<br>`DELETE /talent/:talentId/saved/:itemType/:itemId`<br>`GET /talent/:talentId/matches` | self/admin | no | REST reload + existing local cached profile/opportunity fallback |
| virtual-office-workspace | core-role | `client/src/components/office/VirtualStartupOfficeWorkspace.jsx` | founder, team-member | `POST /presence/update`<br>`GET /presence/:startupId`<br>`GET/POST /startups/:startupId/activities`<br>`GET/POST /startups/:startupId/wins`<br>`POST /messages/send`<br>`GET /messages/conversation/:startupId/:userId/:otherUserId`<br>`GET /messages/conversations/:startupId/:userId`<br>`POST /messages/mark-read`<br>`GET /agenda/:startupId`<br>`GET /calendar/:userId` | startup (with self/admin checks on user-scoped mutations) | yes (`presence`, `activity`, `wins`, `messages`, `tasks`) | Socket fallback to bounded polling + REST requests |
