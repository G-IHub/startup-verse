...
- **The score ring appears in exactly one place on the V2 dashboard now: the hero.** The stat-tile row's first tile and the right panel both used to have their own (a pre-existing duplication, now fixed for the tile row — right panel's score card is untouched, still has its own ring + breakdown, which is a deliberate design difference from the mockup's dashboard right column, not yet reconciled).
