...
- **Read project context**, the source of truth for the stack and community skills. These are different nesting levels — load each that applies (do not treat one as a substitute for the other):
  1. **Root** `AGENTS.md` (fall back to `CLAUDE.md`, else MISSING) — repo-wide stack and conventions.
  2. **Workspace** (monorepo only): when `$SPEC_DIR` is `docs/specs/<workspace>/` (or the topic/path/scope row names a workspace such as `client/`, `server/`, `apps/*`, `packages/*`), **also read that workspace's nested `AGENTS.md`** (e.g. `client/AGENTS.md`, `packages/api/AGENTS.md`) before Framing. Workspace stacks often differ from the root; Framing must infer platform/stack from this file, not assume the root stack. If the workspace has no `AGENTS.md`, note that and fall back to root for that workspace.
  3. **Area** (optional, code-level): the nested `<area>/AGENTS.md` for this feature's area inside the target workspace if one exists (e.g. `client/src/auth/AGENTS.md` for an auth feature). Area ≠ workspace — an area file never replaces a workspace `AGENTS.md`.
...
**Community skills** come from the project's `AGENTS.md`, never a hardcoded name table (names and stacks change). Project wide skills/conventions live in root `AGENTS.md`, workspace-specific ones in the workspace `AGENTS.md` (monorepo), area-specific ones in the nested `<area>/AGENTS.md` (maintained by `/audit` and `/sync`):

1. Read root `AGENTS.md`, the workspace `AGENTS.md` when this is a monorepo workspace feature (already loaded in Pre-flight), and the nested `AGENTS.md` for this feature's area if present; their `## Agent skills` section lists each installed skill as a bullet with its location and a one line note on what it governs, so you can pick out the relevant ones and their paths directly.
...
**References and links: `REFERENCES_LEVEL` must be set before you write.** Never invent or assume a level.

- **If Stage (c) ran** (full staged conversation): reuse that panel's `REFERENCES_LEVEL`. Write the References section and `(basis: ...)` citations only at the chosen level: `none` = no `## References` section and no citations anywhere (Rationale stays); `sources` = named Project sources and Practices only, no links; `sources+links` = sources plus the web verified links the Stage (c) landscape / tool discovery checks already returned. Do NOT fetch anything now; those checks ran once during the conversation and the links they confirmed are what you write. If a link you want was never verified in that check, cite the source by name with no URL rather than fetching at write time. The written links are for a human to follow, not for later AI reads.
- **If Stage (c) never ran** (documentation path, or any path that skipped the staged conversation): `REFERENCES_LEVEL` is **unset**. You must **actively present the References consent panel now** (same panel as Stage (c); recommended pick `No references, keep it clean`), wait for the engineer's answer, then set `REFERENCES_LEVEL` to `none` or `sources` only — do **not** offer `sources+links` (no fetch at write time). Do not skip this panel, do not default silently, and do not treat "Stage (c) was skipped" as "Stage (c) already answered."
...
3b. The References level → `REFERENCES_LEVEL` (`none` | `sources` | `sources+links`). Must already be set: from Stage (c), or from the write-phase consent panel on the documentation path (see References rule above). Never leave this unset; never silently default.
4. Context file contents: `AGENTS.md` (root + workspace when monorepo + the feature area's nested), or `CLAUDE.md` as fallback, or "MISSING"
